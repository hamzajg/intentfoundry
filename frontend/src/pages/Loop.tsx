...
import { Badge, Button, Card, Input, Modal, Spinner, Textarea, Select } from '../components/ui';
...
export function Loop() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <Modal open={showUpdate} onClose={() => setShowUpdate(false)} title="Update Iteration" size="lg">
            ...
            {checkpoints.map((cp) => (
              <div key={cp.id} className="p-3 bg-foundry-800 rounded">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={cp.status === 'approved' ? 'success' : cp.status === 'skipped' ? 'neutral' : cp.status === 'rejected' ? 'error' : 'amber'}>{cp.status}</Badge>
                </div>
      ...
      <Modal open={!!showResolveCheckpoint} onClose={() => setShowResolveCheckpoint(null)} title="Resolve Checkpoint" size="lg">
      ...
      <Modal open={showCreateCheckpoint} onClose={() => setShowCreateCheckpoint(false)} title="Create Checkpoint" size="lg">
        <form onSubmit={handleCreateCheckpoint} className="space-y-4">
          <Input label="Title" value={newCheckpoint.title} onChange={(e) => setNewCheckpoint({ ...newCheckpoint, title: e.target.value })} placeholder="Checkpoint title" required />
          <Textarea label="Description" value={newCheckpoint.description} onChange={(e) => setNewCheckpoint({ ...newCheckpoint, description: e.target.value })} placeholder="Checkpoint description" />
          <div>
          ...
          <label className="flex items-center gap-2">
            ...
            />
            <span className="text-sm text-foundry-100">Required</span>
          </label>
          <div className="flex justify-end gap-3 pt-4">
      ...
      <Modal open={showForceAdvance} onClose={() => setShowForceAdvance(false)} title="Force Advance" size="lg">
      ...
      <Modal open={showAbandon} onClose={() => setShowAbandon(false)} title="Abandon Iteration" size="lg">
